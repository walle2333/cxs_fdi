...
- **链路状态交互**：处理链路激活请求(cxs_rx_active_req)和停用提示(cxs_rx_deact_hint)，输出激活确认(cxs_rx_active)用于发送门控
...
| 高性能数据发送 | 支持256/512/1024/2048位Flit宽度，每周期处理一个Flit | P0 | 输出通路无流水线停顿 |
...
|------|--------|------|------|-------------|
| 工作频率 | 1.5 | GHz | 与CXS接口同步 | 时序关键路径约束 |
...
| CXS输出 | cxs_rx_user | Output | [CXS_USER_WIDTH] | CXS | cxs_clk | 用户定义位(可选)，位宽0-128可配置 |
...
| CXS输出 | cxs_rx_active | Output | 1 | CXS | cxs_clk | 链路激活确认(可选) |
...
| cxs_rx_data | **CXSDATA** | Output | [CXS_DATA_WIDTH] | 数据载荷。支持256/512/1024/2048位，本设计默认512位 | 寄存器输出，与valid同步 |
| cxs_rx_user | **CXSUSER** | Output | [CXS_USER_WIDTH] | 用户定义位。**可选信号**，用于传递协议层扩展信息。位宽0-128可配置，0表示不包含 | 寄存器输出 |
| cxs_rx_cntl | **CXSCNTL** | Output | [CXS_CNTL_WIDTH] | 控制字段(含ENDERROR位用于错误标记) | 寄存器输出；位宽由CXS_DATA_WIDTH/CXS_MAX_PKT_PER_FLIT/CXS_START_ALIGNMENT决定；当`CXS_MAX_PKT_PER_FLIT=1`时，`CXS_CNTL_WIDTH`必须为0，此信号不存在 |
| cxs_rx_last | **CXSLAST** | Output | 1 | 包边界指示(可选) | 寄存器输出，默认高；当`CXS_MAX_PKT_PER_FLIT=1`时，`CXS_HAS_LAST`必须为0，此信号不存在 |

**参数规则补充：**
- 当 `CXS_MAX_PKT_PER_FLIT=1` 时，`CXS_CNTL_WIDTH` 必须为 0，`CXS_HAS_LAST` 必须为 0
...
| cxs_rx_active | **CXSACTIVEACK** | Output | 1 | 链路激活确认 | 高电平有效，由Link Control FSM提供并输出给协议层 |
...
| 时钟名称 | 频率 | 描述 | RTL实现要求 |
|----------|------|------|-------------|
| cxs_clk | 1.5 GHz (1.5) | CXS侧协议时钟 | 所有时序逻辑使用上升沿 |
...
- **RTL职责**：透传并响应链路激活/停用信号
- **状态管理**：不实现独立链路状态机，状态由`cxs_fdi_link_ctrl`统一提供

#### CXS输出封装单元 (CXS Output Packing Unit)
- **模块名称**: CXS Output Packing Unit
- **RTL职责**：将数据封装为CXS协议格式输出

### 5.2 链路门控行为 / Link Gating Behavior

本模块不实现独立状态机。链路状态机定义见`cxs_fdi_link_ctrl_spec.md`。

**本模块链路门控行为：**
| 条件 | 行为 |
|------|------|
| `cxs_rx_active=1` | 允许输出`cxs_rx_valid` |
| `cxs_rx_active=0` | 抑制输出，保持空闲 |
