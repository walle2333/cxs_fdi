...
- **DLLP接收**：接收fdi_pl_dllp_valid和fdi_pl_dllp传输链路管理信息
...
| 高性能数据接收 | 支持512B Flit宽度，满带宽接收 | P0 | Ready/Valid握手效率最大化 |
...
|------|--------|------|------|-------------|
| 工作频率 | 2.0 | GHz | 与FDI链路同步 | 时序约束 |
| 吞吐量 | 128 | GB/s | 512B × 2.0GHz | 数据通路需无气泡 |
...
|----------|----------|------|------|------|--------|-------------|
| 时钟复位 | fdi_lclk | Input | 1 | - | - | 全局时钟，上升沿采样，2.0GHz |
...
| FDI输入 | fdi_pl_dllp_valid | Input | 1 | FDI | fdi_lclk | DLLP有效 |
| FDI输入 | fdi_pl_dllp | Input | [FDI_DLLP_WIDTH] | FDI | fdi_lclk | DLLP数据 |
...
|--------|-----------|------|------|------|-------------|
| fdi_pl_valid | **fdi_pl_valid** | Input | 1 | Protocol Layer到Adapter的接收数据有效指示 | 与lclk同步 |
...
| fdi_pl_dllp | **fdi_pl_dllp** | Input | [FDI_DLLP_WIDTH] | 接收DLLP数据 | 与valid同步 |
...
| 时钟名称 | 频率 | 描述 | RTL实现要求 |
|----------|------|------|-------------|
| fdi_lclk | 2.0 GHz (1.5~2.0) | FDI侧链路时钟 | 所有时序逻辑使用上升沿 |

### 4.2 跨时钟域 / Clock Domain Crossings (CDC)

本模块为单时钟域模块(fdi_lclk)，不涉及内部CDC。CDC由Async FIFO处理。
...
- **数据处理**：
  ```systemverilog
  // Ready生成 - 可提前断言
  assign fdi_pl_trdy = rx_ready && link_active && !flit_cancel_pending;
  ...
  always_ff @(posedge fdi_lclk or negedge fdi_rst_n) begin
    if (!fdi_rst_n) begin
      ...
      rx_ready <= !rx_ready || rx_valid_reg;  // 有空间则Ready
    end
  end
  ```
...
| 状态 | 编码 | 描述 | RTL退出条件 |
|------|------|------|-------------|
| IDLE | 2'b00 | 链路未就绪 | link_up==1 |
| READY | 2'b01 | 链路就绪，等待数据 | valid && ready |
| RECEIVE | 2'b10 | 接收数据 | 数据完成 |
| PAUSE | 2'b11 | 暂停(链路错误) | error_clear |
...
| 0x02 | ERR_PHY_ERROR | 物理层错误 | fdi_pl_error检测 |
| 0x03 | ERR_FLIT_CANCEL | Flit取消 | flit_cancel检测 |
...
|----------|----------|------|----------|------|
| FDI_RX_CTRL | 0x00 | 32-bit | R/W | FDI RX控制寄存器 |
| FDI_RX_STATUS | 0x04 | 32-bit | R | FDI RX状态寄存器 |
| FDI_RX_CONFIG | 0x08 | 32-bit | R/W | FDI RX配置寄存器 |
| FDI_RX_ERR_STATUS | 0x14 | 32-bit | R/W1C | 错误状态寄存器 |
