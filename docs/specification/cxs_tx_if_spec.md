...
- **链路状态交互**：处理链路激活请求(CXSACTIVEREQ)和停用提示(CXSDEACTHINT)，输出激活确认(CXSACTIVEACK)用于发送门控
...
| 高性能数据接收 | 支持256/512/1024/2048位Flit宽度，每周期处理一个Flit，吞吐量达到满带宽 | P0 | 数据通路无流水线停顿，输入寄存器直接采样 |
...
|------|--------|------|------|-------------|
| 工作频率 | 1.5 | GHz | 与CXS接口同步 | 时序关键路径约束在单周期内 |
| 吞吐量 | 96 | GB/s | 512bit × 1.5GHz | 数据通路需无气泡 |
...
|----------|----------|------|------|------|--------|-------------|
| 时钟复位 | cxs_clk | Input | 1 | - | - | 全局时钟，上升沿采样，1.5GHz |
...
| CXS输入 | cxs_tx_user | Input | [CXS_USER_WIDTH] | 可选 | CXS | cxs_clk | 用户定义位，位宽0-128可配置 |
...
| CXS输入 | cxs_tx_active_req | Input | 1 | 可选 | CXS | cxs_clk | 链路激活请求 |
| CXS输出 | cxs_tx_active | Output | 1 | 可选 | CXS | cxs_clk | 链路激活确认 |
| CXS输入 | cxs_tx_deact_hint | Input | 1 | 可选 | CXS | cxs_clk | 链路停用提示 |
...
| cxs_tx_data | **CXSDATA** | Input | [CXS_DATA_WIDTH] | 数据载荷。支持256/512/1024/2048位，本设计默认512位 | 与CXSVALID同步，在CXSVALID高时采样 |
| cxs_tx_user | **CXSUSER** | Input | [CXS_USER_WIDTH] | 用户定义位。**可选信号**，用于传递协议层扩展信息。位宽0-128可配置，0表示不包含 | 与CXSVALID同步采样 |
| cxs_tx_cntl | **CXSCNTL** | Input | [CXS_CNTL_WIDTH] | 控制字段。包含START[x:0]、END[x:0]、ENDERROR[x:0]、STARTxPTR、ENDxPTR等子字段。**ENDERROR位用于错误标记(替代独立POISON信号)** | 与CXSVALID同步，位宽由CXS_DATA_WIDTH/CXS_MAX_PKT_PER_FLIT/CXS_START_ALIGNMENT决定；当`CXS_MAX_PKT_PER_FLIT=1`时，`CXS_CNTL_WIDTH`必须为0，此信号不存在 |
| cxs_tx_last | **CXSLAST** | Input | 1 | 包边界指示。表示当前Flit后可插入其他协议数据，或表示包结束。**可选信号**，不存在时默认为高 | 与CXSVALID同步；当`CXS_MAX_PKT_PER_FLIT=1`时，`CXS_HAS_LAST`必须为0，此信号不存在 |
...
| cxs_tx_active | **CXSACTIVEACK** | Output | 1 | 链路激活确认。表示TX路径已就绪可接收数据 | 高电平有效，由Link Control FSM提供并输出给协议层 |
...
| 信号名 | CXS标准名 | 方向 | 位宽 | 描述 | RTL实现要求 |
|--------|-----------|------|------|------|-------------|
| cxs_tx_srcid | **CXSSRCID** | Input | [CXS_SRCID_WIDTH] | 源ID标识。用于多节点拓扑，可配置宽度0-8位 | 与CXSVALID同步，可选信号 |
| cxs_tx_tgtid | **CXSTGTID** | Input | [CXS_TGTID_WIDTH] | 目标ID标识。用于路由，可配置宽度0-8位 | 与CXSVALID同步，可选信号 |
...
| 时钟名称 | 频率 | 描述 | RTL实现要求 |
|----------|------|------|-------------|
| cxs_clk | 1.5 GHz (1.5) | CXS侧协议时钟，与CXS接口同步 | 所有时序逻辑使用上升沿 |

**时钟关系：**
- cxs_clk由外部时钟源提供，频率范围1.5GHz
- 本模块所有逻辑都在cxs_clk域
...
| cxs_tx_active | ← | 链路激活确认（来自cxs_fdi_link_ctrl，输出给协议层） |
...
|----------|----------|------|-------------|
| [0] | ERR_CXS_PROTO | CXS协议违例 | 时序/稳定性检查 |
| [5] | ERR_LINK_TIMEOUT | 链路激活超时 | 激活请求超时计数器 |
...
**与本模块相关的全局寄存器/字段：**
- `STATUS.TX_READY`、`STATUS.LINK_STATE`
- `ERR_STATUS.ERR_CXS_PROTO`
- `TX_FLIT_CNT_L/H`（发送Flit计数）
- `CTRL.FLIT_WIDTH`（与协议层Flit宽度配置保持一致）
...
| 指标 | 目标值 | 单位 | RTL实现约束 |
|------|--------|------|-------------|
| 峰值吞吐量 | 96 | GB/s | 512bit × 1.5GHz = 96GB/s |
| 持续吞吐量 | 96 | GB/s | 每周期一个Flit，无气泡 |
...
| 条件 | 超时值 | RTL处理动作 |
|------|--------|--------------|
| 激活请求超时 | 1024周期 | 上报ERR_LINK_TIMEOUT，链路保持非激活并等待Link Control恢复 |
...
1. **问题定位**：通过`STATUS`/`ERR_STATUS`确定链路状态和错误状态
...
// 模块参数定义
// 注意: CXS协议中错误标记通过CXSCNTL的ENDERROR位实现，无独立的POISON信号
// 参数规则: 当 CXS_MAX_PKT_PER_FLIT=1 时，CXS_CNTL_WIDTH 必须为0，CXS_HAS_LAST 必须为0
module cxs_tx_if #(
  parameter integer CXS_DATA_WIDTH     = 512,  // CXS数据宽度
  parameter integer CXS_USER_WIDTH     = 64,   // 用户位宽度 (0=不包含)
  parameter integer CXS_CNTL_WIDTH     = 18,   // 控制字段宽度(512b/2pkt/16B对齐)
  parameter integer CXS_MAX_PKT_PER_FLIT = 2,  // 每Flit最大包数
  parameter integer CXS_START_ALIGNMENT  = 16, // 起始对齐(Byte)
  ...
  parameter integer CXS_USER_WIDTH_E  = (CXS_USER_WIDTH  > 0) ? CXS_USER_WIDTH  : 1,
  parameter integer CXS_SRCID_WIDTH_E = (CXS_SRCID_WIDTH > 0) ? CXS_SRCID_WIDTH : 1,
  parameter integer CXS_TGTID_WIDTH_E = (CXS_TGTID_WIDTH > 0) ? CXS_TGTID_WIDTH : 1
) (
  // 时钟复位
  input  logic                 cxs_clk,
  ...
  output logic [CXS_USER_WIDTH_E-1:0] tx_user_out,
  output logic [CXS_CNTL_WIDTH-1:0]  tx_cntl_out,  // 包含ENDERROR位透传
  output logic                 tx_last_out,
  output logic [CXS_SRCID_WIDTH_E-1:0] tx_srcid_out,
  output logic [CXS_TGTID_WIDTH_E-1:0] tx_tgtid_out,
...
// 当可选位宽参数为0时，*_WIDTH_E用于保留可综合端口宽度
```
