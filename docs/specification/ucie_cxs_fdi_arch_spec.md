...
- **高性能传输**: 支持可配置Flit宽度(256b/512b/1Kb/2Kb)，保证满带宽传输，最小化跨桥延迟
...
| 全双工高性能 | 支持256b/512b/1Kb/2Kb Flit宽度，满带宽传输 | P0 |
...
|------|--------|------|------|
| 工艺节点 | 7nm | - | TSMC 7nm工艺 |
| 工作频率 | 2.0 | GHz | 最高频率，与CXS/FDI接口同步 |
...
│  │  cxs_tx_crdgnt ◄─────────────────────┤ (来自Credit Manager)                               │   │
│  │  cxs_tx_crdret ──────────────────────│▶ (送至Credit Manager)                             │   │
...
4. **FDI TX Interface (fdi_lclk域)**：将数据封装为fdi_lp_valid, fdi_lp_flit, fdi_lp_stream发送给UCIe Adapter
5. **FDI RX Interface (fdi_lclk域)**：接收来自Adapter的fdi_pl_valid, fdi_pl_flit, fdi_pl_stream
...
| 接口名称 | 方向 | 位宽 | 时钟域 | 描述 |
|----------|------|------|--------|------|
| cxs_clk | Input | 1 | - | CXS侧协议时钟，2.0GHz |
| cxs_rst_n | Input | 1 | cxs_clk | CXS侧异步复位，低电平有效，同步释放 |
| fdi_lclk | Input | 1 | - | FDI侧链路时钟，与cxs_clk同频(2.0GHz)，相位独立 |
...
| cxs_tx_data | **CXSDATA** | Input | [CXS_DATA_WIDTH] | 必需 | 数据载荷。支持8-2048位，本设计默认512位 |
| cxs_tx_cntl | **CXSCNTL** | Input | [CXS_CNTL_WIDTH] | 必需 | 控制字段。包含START[x:0]、END[x:0]、ENDERROR[x:0]、STARTxPTR、ENDxPTR等子字段。ENDERROR位用于错误标记(替代独立POISON信号) |
...
|--------|-----------|------|------|------|----------|
| cxs_rx_valid | **CXSVALID** | Output | 1 | 发送端Flit有效指示。高电平时表示CXSDATA有效 | 必须在数据有效前建立，保持到数据结束 |
| cxs_rx_data | **CXSDATA** | Output | [CXS_DATA_WIDTH] | 数据载荷。支持8-2048位，本设计默认512位 | 与CXSVALID同步 |
| cxs_rx_user | **CXSUSER** | Output | [CXS_USER_WIDTH] | 可选 | 用户定义位。用于传递协议层扩展信息 | 与CXSVALID同步 |
| cxs_rx_cntl | **CXSCNTL** | Output | [CXS_CNTL_WIDTH] | 必需 | 控制字段。包含START[x:0]、END[x:0]、ENDERROR[x:0]、STARTxPTR、ENDxPTR等子字段，用于包边界指示和错误标记 | 与CXSVALID同步，位宽根据CXS_DATA_WIDTH和CXS_MAX_PKT_PER_FLIT确定 |
| cxs_rx_last | **CXSLAST** | Output | 1 | 可选 | 包边界指示。表示当前Flit后可插入其他协议数据，或表示包结束 | 默认高 |
| cxs_rx_active | **CXSACTIVEACK** | Output | 1 | 可选 | 链路激活确认。表示RX路径已就绪可接收数据 | 高电平有效 |
| cxs_rx_active_req | **CXSACTIVEREQ** | Input | 1 | 可选 | 链路激活请求。协议层请求激活链路 | 高电平有效 |
...
| fdi_lp_dllp_valid | **fdi_lp_dllp_valid** | Output | 1 | DLLP有效指示。用于低延迟DLLP传输，可与Flit同时有效 | 与fdi_lp_valid同时或独立断言 |
| fdi_lp_dllp | **fdi_lp_dllp** | Output | [FDI_DLLP_WIDTH] | DLLP数据。用于链路管理DLLP传输 | 与fdi_lp_dllp_valid同步 |
...
| 信号名 | FDI标准名 | 方向 | 位宽 | 描述 | 时序要求 |
|--------|-----------|------|------|------|----------|
| fdi_pl_valid | **fdi_pl_valid** | Input | 1 | Protocol Layer到Adapter的接收数据有效指示 | 与lclk同步，高电平时fdi_pl_flit有效 |
...
| fdi_pl_dllp_valid | **fdi_pl_dllp_valid** | Input | 1 | 接收DLLP有效 | 可与fdi_pl_flit同时有效 |
| fdi_pl_dllp | **fdi_pl_dllp** | Input | [FDI_DLLP_WIDTH] | 接收DLLP数据 | 与fdi_pl_dllp_valid同步 |
...
| CXSCRDGNT | 3.1 | fdi_lp_irdy | 10.2 | Credit→Ready映射(由credit_mgr驱动) |
...
1. **CXS Credit机制 → FDI Ready/Valid握手：**
   ...
   - 映射关系：`credit_mgr` 维护Credit计数器，当Credit>0时驱动/门控fdi_lp_irdy相关流控
...
2. **Explicit Credit Return模式：**
   ...
   - `credit_mgr` 在收到CXSCRDRTN后更新Credit计数，并影响fdi_lp_irdy相关流控
...
| Credit Grant | CXSCRDGNT | fdi_lp_irdy + Credit Count | C2C Credit映射到CXS和FDI流控(由credit_mgr维护) |
...
1. **C2C-Specific Message映射**：
   ...
   - 这些消息通过CXS的User bits传递，或通过专用sideband通道
   - Bridge需要识别并正确处理这些消息，不将其作为普通数据转发到FDI

2. **Credit管理映射**：
   - CHI C2C Credit机制与CXS Credit机制直接对应
   - C2C Credit Grant消息映射到CXSCRDGNT信号
   - FDI方向使用fdi_lp_irdy与`credit_mgr`维护的Credit计数器实现等效流控
   - 注意：CHI C2C Credit是针对消息类型的，Bridge需要维护多个Credit计数器(由credit_mgr承担)

3. **Link State管理**：
   - CHI C2C Link State与FDI fdi_pl_state_sts有直接映射关系
   - STOP/ACTIVREQ/ACTIVE/DEACT等C2C状态对应到FDI的Reset/LinkUp/Active/Retrain等状态
...
- `credit_mgr` 维护Credit计数器，根据可用Credit驱动/门控fdi_lp_irdy相关流控
- 当接收到FDI的fdi_pl_flit时，`credit_mgr` 通过CXSCRDGNT完成Credit授权
...
|----------|----------|-------------|----------------|
| 数据信号 | CXSLAST | 可选 (Y) | **配置选择**: 通过参数 `CXS_HAS_LAST` 决定是否包含 |
| 数据信号 | CXSUSER | 可选 (Y) | **配置选择**: 通过参数 `CXS_USER_WIDTH` 决定位宽，0表示不包含 |
| ID信号 | CXSSRCID | 可选 (Y) | **配置选择**: 通过参数 `CXS_SRCID_WIDTH` 决定位宽，0表示不包含 |
| ID信号 | CXSTGTID | 可选 (Y) | **配置选择**: 通过参数 `CXS_TGTID_WIDTH` 决定位宽，0表示不包含 |
...
| 字段 | 位宽 | 描述 | 本设计处理 |
|------|------|------|-----------|
| START[x:0] | x=CXSMAXPKTPERFLIT-1 | 包开始指示 | 直接透传到FDI Flit Header |
| END[x:0] | x=CXSMAXPKTPERFLIT-1 | 包结束指示 | 直接透传到FDI Flit Header |
| ENDERROR[x:0] | x=CXSMAXPKTPERFLIT-1 | 包错误标记 (替代POISON) | **映射到FDI Flit的Error字段** |
...
module cxs_tx_if #(
    // 必需参数
    parameter integer CXS_DATA_WIDTH    = 512,
    parameter integer CXS_CNTL_WIDTH    = 16,    // 根据MAXPKTPERFLIT计算
    
    // 可选信号配置
    parameter bit    CXS_HAS_LAST      = 1'b1,  // 1=包含CXSLAST, 0=不包含
    parameter integer CXS_USER_WIDTH   = 64,     // 0=不包含, >0=包含并指定位宽
    parameter integer CXS_SRCID_WIDTH  = 8,      // 0=不包含, 1-8=包含并指定位宽
    parameter integer CXS_TGTID_WIDTH  = 8,      // 0=不包含, 1-8=包含并指定位宽
    ...
    // 可选信号 - 通过generate语句条件生成
    generate
        if (CXS_HAS_LAST) begin : gen_cxs_last
            input  logic cxs_tx_last;
        end
    endgenerate
    
    generate
        if (CXS_USER_WIDTH > 0) begin : gen_cxs_user
...
```systemverilog
// CXSCNTL字段解析
// CXSCNTL格式 (以CXSMAXPKTPERFLIT=2为例):
...
// ENDERROR位提取
logic [MAXPKTPERFLIT-1:0] enderror_bits;
assign enderror_bits = CXS_CNTL_HAS_ENDERROR ? cxs_tx_cntl[ENDERROR_LSB +: MAXPKTPERFLIT] : '0;

// ENDERROR到FDI映射
// 在FDI Flit Header中包含Error位，映射CXS的ENDERROR
assign fdi_lp_flit_header.error = |enderror_bits;  // 任一包有错则置位

// 或者保持ENDERROR在数据载荷中透传
assign fdi_lp_flit[ERROR_POS +: MAXPKTPERFLIT] = enderror_bits;
```
...
|----------|------|----------|------|----------|
| cxs_clk | 2.0 GHz | 1.5~2.0 GHz | CXS侧协议时钟，与CXS接口同步 | CXS TX/RX接口、TX/RX Path Logic、Credit Manager、Link Ctrl FSM |
| fdi_lclk | 2.0 GHz | 1.5~2.0 GHz | FDI侧链路时钟，与UCIe链路同步 | FDI TX/RX接口 |
...
| cxs_clk | fdi_lclk | 控制信号 | 2级同步器 | 链路状态更新 |
| fdi_lclk | cxs_clk | 状态信号 | 2级同步器 | FDI状态反馈 |
...
| rst_n_cxs | Low | 局部 | 同步 | cxs_clk域 | CXS接口复位 |
| rst_n_fdi | Low | 局部 | 同步 | fdi_lclk域 | FDI接口复位 |
| rst_sw | High | 软件 | 同步 | 全局 | 软件复位 |

**复位顺序：**
1. 异步复位断言 (rst_n = 0)
2. 同步释放 rst_n (延迟3个时钟周期)
3. 释放 rst_n_cxs (延迟1个时钟周期)
4. 释放 rst_n_fdi (延迟1个时钟周期)
...
- **输出**: fdi_lp_valid, fdi_lp_flit, fdi_lp_stream等 (FDI TX Interface输出)
...
- **输入**: fdi_pl_valid, fdi_pl_flit, fdi_pl_stream等 (FDI RX Interface输入)
...
- **职责**: 管理链路激活/停用状态
- **输入**: activation/deactivation请求, FDI状态信号
- **输出**: 链路状态信号
- **处理流程**: 状态转换 → 超时处理 → 重试机制
- **子模块**:
...
- **输入**: fdi_pl_valid, fdi_pl_flit, fdi_pl_stream, fdi_pl_dllp, fdi_pl_flit_cancel等
- **输出**: rx_valid_out, rx_data_out, rx_user_out等
- **关键特性**: 
  - Ready/Valid握手接收
  - Flit Cancel处理
  - DLLP支持
...
4. 数据在FIFO中缓冲，不丢弃
5. 等待链路恢复 (fdi_pl_state_sts = Active)
6. 恢复数据发送，FIFO数据按序发送
...
| 0x03 | ERR_FIFO_OVERFLOW | FIFO溢出 | 中 | 停止写入，记录错误 |
| 0x04 | ERR_FIFO_UNDERFLOW | FIFO下溢 | 中 | 停止读取，记录错误 |
| 0x05 | ERR_LINK_DOWN | 链路断开 | 高 | 进入停止状态，记录错误 |
| 0x06 | ERR_LINK_TIMEOUT | 链路激活超时 | 中 | 重试激活，记录错误 |
...
| FIFO溢出 | 暂停写入，等待FIFO空间 | 可变 | 防止数据丢失 |
...
| [7:4] | FLIT_WIDTH | R/W | 0x1 | Flit宽度：0=256b,1=512b,2=1Kb,3=2Kb |
| [3:1] | RESERVED3 | R/W | 0x00 | 保留 |
| [0] | RESET | R/W1S | 0 | 软件复位：写1复位模块 |
...
| [23:16] | FIFO_DEPTH | R/W | 0x40 | FIFO深度配置 (8-128) |
| [15:8] | TIMEOUT | R/W | 0xFF | 超时计数(单位：时钟周期) |
| [7:0] | RETRY_CNT | R/W | 0x03 | 最大重试次数 |
...
| [2] | ERR_FIFO_OVERFLOW | R/W1C | 0 | FIFO溢出 |
...
| 指标 | 目标值 | 单位 | 测试条件 | 测试方法 |
|------|--------|------|----------|----------|
| 峰值吞吐量 | 128 | GB/s | 2Kb Flit, 2GHz | 持续满带宽传输 |
| 持续吞吐量 | 120 | GB/s | 长时间运行 | 压力测试 |
| 跨桥延迟 | 10 | ns | 无竞争 | 端到端测量 |
| 带宽利用率 | > 95% | % | AXI满载 | 统计计数器 |
...
| 指标 | 目标值 | 单位 | 条件 |
|------|--------|------|------|
| 突发带宽 | 128 | GB/s | 突发长度=16 |
| 突发延迟 | 20 | ns | 突发长度=16 |
| 启动延迟 | 15 | 时钟周期 | 从IDLE到BUSY |
| 关闭延迟 | 10 | 时钟周期 | 从BUSY到IDLE |
...
| Flit宽度配置 | 定向 | 256b/512b/1Kb/2Kb均正确 | P0 |
...
| CXS_USER_WIDTH | integer | 64 | [32, 64, 128] | CXS用户定义位宽度 |
| CXS_SRCID_WIDTH | integer | 8 | [0, 8, 16] | CXS源ID位宽，0表示不包含 |
| CXS_TGTID_WIDTH | integer | 8 | [0, 8, 16] | CXS目标ID位宽，0表示不包含 |
| CXS_CNTL_WIDTH | integer | 8 | [4, 8, 16] | CXS控制字段宽度 |
| FDI_DATA_WIDTH | integer | 512 | [256, 512, 1024, 2048] | FDI Flit宽度 |
| FDI_USER_WIDTH | integer | 64 | [32, 64, 128] | FDI用户定义位宽度 |
| FDI_STREAM_WIDTH | integer | 4 | [2, 4, 8] | FDI流ID位宽 |
| FDI_DLLP_WIDTH | integer | 32 | [16, 32, 64] | FDI DLLP数据位宽 |
| FIFO_DEPTH | integer | 64 | [32, 64, 128, 256] | 异步FIFO深度 |
| MAX_CREDIT | integer | 32 | [8, 16, 32, 64] | 最大Credit数量 |
...
UCIe CXS-FDI Bridge工作在高速接口（2GHz），需要严格的时序约束以确保信号完整性。

#### 15.1.1 时钟约束

| 时钟名 | 频率 | 周期 | 占空比 | 不确定性 | 抖动 | 源延迟 |
|--------|------|------|--------|----------|------|--------|
| cxs_clk | 2.0 GHz | 500 ps | 50% ± 5% | 20 ps | 10 ps | 50 ps |
| fdi_lclk | 2.0 GHz | 500 ps | 50% ± 5% | 20 ps | 10 ps | 50 ps |
...
- **水平层**：M1, M3, M5
- **垂直层**：M2, M4, M6
- **电源网络**：VDD_CORE (0.8V), VDD_AXI (0.9V), VSS (0.0V)
