...
- **流控处理**：检测fdi_lp_irdy信号，根据Ready状态发送数据
- **多流支持**：通过fdi_lp_stream信号标识不同的协议流
- **DLLP传输**：支持fdi_lp_dllp_valid和fdi_lp_dllp传输链路管理信息
...
| 高性能数据发送 | 支持512B Flit宽度，满带宽传输 | P0 | Ready/Valid握手效率最大化 |
...
| FDI输出 | fdi_lp_dllp_valid | Output | 1 | FDI | fdi_lclk | DLLP有效 |
| FDI输出 | fdi_lp_dllp | Output | [FDI_DLLP_WIDTH] | FDI | fdi_lclk | DLLP数据 |
...
- fdi_lp_irdy可提前fdi_lp_valid最多2个周期断言（Adapter可提前准备）
 - fdi_lp_irdy 由 Adapter 驱动，但可被 credit_mgr 的 credit_ready 信号门控/协同
...
| 信号名 | FDI标准名 | 方向 | 位宽 | 描述 | RTL实现要求 |
|--------|-----------|------|------|------|-------------|
| fdi_lp_dllp_valid | **fdi_lp_dllp_valid** | Output | 1 | DLLP有效指示 | 可与Flit同时有效 |
| fdi_lp_dllp | **fdi_lp_dllp** | Output | [FDI_DLLP_WIDTH] | DLLP数据 | 与valid同步 |
...
- **核心逻辑**：
  ...
  assign fdi_lp_valid = tx_valid_reg && fdi_lp_irdy && link_active;
...
- **状态处理**：
  ...
  assign fdi_lp_valid = tx_valid_reg && fdi_lp_irdy && !send_pause;
  ```
...
| 状态 | 编码 | 描述 | RTL退出条件 |
|------|------|------|-------------|
| IDLE | 2'b00 | 链路未激活 | link_up==1 |
| READY | 2'b01 | 链路就绪 | valid && ready |
| SEND | 2'b10 | 发送数据 | !valid || retrain |
| PAUSE | 2'b11 | 暂停(Retrain) | retrain_clear |
...
| 0x02 | ERR_RETRAIN_TIMEOUT | Retrain超时 | 计数器超时 |
...
|----------|----------|------|----------|------|
| FDI_TX_CTRL | 0x00 | 32-bit | R/W | FDI TX控制寄存器 |
| FDI_TX_STATUS | 0x04 | 32-bit | R | FDI TX状态寄存器 |
| FDI_TX_CONFIG | 0x08 | 32-bit | R/W | FDI TX配置寄存器 |
| FDI_TX_ERR_STATUS | 0x14 | 32-bit | R/W1C | 错误状态寄存器 |
